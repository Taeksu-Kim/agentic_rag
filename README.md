...
일상어 질문을 **검색(RAG)** 으로 답하는 데서 그치지 않고, **연차 계산·신청 같은 처리 업무**까지 **에이전트가 도구를 호출해** 수행합니다. 신청은 **사람이 승인**해야 제출됩니다(HITL). _아래 GIF는 실시간 속도 — 로컬 4090 1장에서 9B 에이전트가 실제로 도는 시간._

![데모 GIF](docs/media/demo.gif)

하나의 대화가 **복합 질문(검색) → 계산 → 신청**으로 자연스럽게 이어집니다:
...
**테스트 198개, 전부 fake 기반** (네트워크·실서버·실모델 없이 `pytest` 한 방) — 임베더/리랭커/LLM/웹은 Protocol 뒤에 두고 fake로 갈아끼운다.

## 한계 · 후속

- 에이전트가 리라이팅 쿼리만 쓰지 말고 **원 질문 그대로의 검색을 항상 1회 포함**하면 1-shot 이상이 구조적으로 보장됨 — 시딩으로 실장(`seed_raw_search`), 격차 절반 회복 확인
- **퓨샷 리라이터는 무효과로 기각** (정직한 네거티브): 리라이터를 질적으로 개선해도 검색 지표 불변 — CE 리랭커가 어휘 갭을 이미 흡수해 리라이팅에 남은 헤드룸이 없음을 재확인. 남은 실질 레버는 임베더 체급 ([`docs/ablation_results.md`](docs/ablation_results.md) §퓨샷 리라이터)
- qrels가 회답 인용 조문 전부를 정답으로 치는 보수적 채점 (멀티라벨 완전회수는 1-shot도 21%)
- **유스케이스/누적 데이터 기반 고도화 레버**(doc2query 역질문·HyDE·과거질문 검색·co-citation)는 실서비스 운영 로그가 전제라 본편에서 분리 — 설계·근거만 [`docs/design_and_plan.md`](docs/design_and_plan.md) §10에 기록
